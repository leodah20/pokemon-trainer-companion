...
Full history of implemented features, in reverse-chronological order (newest first). The README
keeps only the 5 most recent entries inline — this file has everything.
...
>   (Bulbapedia-style trivia) — see "Post-beta scope" in the README.
...
>   content (`generateQuiz.ts`), and Pokemon type names (Fire, Water, etc.).
...
> - Deliberately not in this pass: the remaining screens and the lore content itself.
...
>   work not yet started at this point
...
> - Declared this the v1.0 Beta cut.
...
>   Android build has a fragile history this session. Both animated values still run on the
>   native thread (`useNativeDriver: true`), so this isn't a JS-thread fallback — just no
...
>   copies of the same backend data, not live API calls.
...
>   **14/14 passing**
> - Not yet done at this point: wiring these into mobile screens and the scheduled data-sync job
...
> - Raid counters with DPS and the evolution chain viewer were still planned at this point,
>   blocked on data the app didn't have yet (a fast/charged move power+energy+duration database,
>   and evolution-family data beyond ~150 species)
...
>   (mobile doesn't call the backend)
